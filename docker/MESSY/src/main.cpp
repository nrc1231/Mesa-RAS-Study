...
SCA_TDF_MODULE(input_generator)     //TCP socket client
{
...
SCA_TDF_MODULE(active_suspension_module)    //regeneration circuit model
{
    sca_tdf::sca_in<double> r_in;    
    sca_tdf::sca_in<double> fs_in;   

    sca_tdf::sca_out<double> xb_out; 
    sca_tdf::sca_out<double> sd_out; 
    sca_tdf::sca_out<double> ab_out; 
    sca_tdf::sca_out<double> xw_out;
    ...
    if(gamma>=1){
      ...
      Ic=(-ssig*ec-phi*vrel1)/(var+rm);    
      cs=phi*Ic;     //here should be - if using the SH controller
    }
    ...
    if(gamma2>=1){
      ...
      Ic2=(-ssig2*ec-phi*vrel2)/(var2+rm);    
      cs2=phi*Ic2;      //here should be - if using the SH controller
    }
    ...
    if(gamma3>=1){
      ...
      Ic3=(-ssig3*ec-phi*vrel3)/(var3+rm);    
      cs3=phi*Ic3;                  //here should be - if using the SH controller
    }
    ...
    if(gamma4>=1){
      ...
      cs4=phi*Ic4;                               //here should be - if using the SH controller
    }
    Itot=Ic+Ic2+Ic3+Ic4;
    cs=-cs;
    cs2=-cs2;
    ...
    xw_out.write(2);    //Reserved port
    sd_out.write(ps);
    ab_out.write(4);    //Reserved port
}
...
{
    sca_tdf::sca_in<double> xb_in;   
    sca_tdf::sca_in<double> sd_in;   
    sca_tdf::sca_in<double> ab_in;   
    sca_tdf::sca_out<double> out1;   
    ...
    void processing() {
        ...
        SOC=SOC-(((Isum)*0.001)*1.36)/(161*36*2);
...
SCA_TDF_MODULE(additional_energy_cost)       //Reserved module
   {  
    sca_tdf::sca_out<double> out;
